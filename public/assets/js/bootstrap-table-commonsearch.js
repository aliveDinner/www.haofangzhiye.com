            ...
            if (!vObjCol.checkbox && vObjCol.field !== 'operate' && vObjCol.searchable && vObjCol.operate !== false) {
                ...
                if (vObjCol.searchList) {
                    ...
                    if (/BETWEEN$/.test(vObjCol.operate)) {
                        ...
                        if(type==='checkbox')
                        {
                            addclass=typeof vObjCol.addclass === 'undefined' ?"":vObjCol.addclass;
                            htmlForm.push(sprintf('<div class="%s" ><label>',type));
                            htmlForm.push(sprintf('<input type="%s" class="%s" name="%s" value="%s"  id="%s" data-index="%s" %s %s>', type, addclass, vObjCol.field, defaultValue, vObjCol.field, i, style, data));
                            htmlForm.push('</label></div>');
                        }
                        else
                        {
